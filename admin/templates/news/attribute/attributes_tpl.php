...
$linkAdd = "index.php?com=news&act=add_attribute&type=" . $type;
$linkEdit = "index.php?com=news&act=edit_attribute&type=" . $type;
$linkDelete = "index.php?com=news&act=delete_attribute&type=" . $type;
$linkMulti = "index.php?com=news&act=man_photo&kind=man_attribute&type=" . $type;
?>
<!-- Content Header -->
    ...
    <div class="container-fluid">
        <div class="row">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active">Quản lý <?= $config['news'][$type]['title_main_attribute'] ?></li>
            </ol>
        </div>
    </div>
</section>
...
<section class="content">
    ...
    <div class="card card-primary card-outline text-sm mb-0">
        <div class="card-header">
            <h3 class="card-title">Danh sách <?= $config['news'][$type]['title_main_attribute'] ?></h3>
        </div>
            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <?php if (isset($config['news'][$type]['show_images_attribute']) && $config['news'][$type]['show_images_attribute'] == true) { ?>
                            <th class="align-middle">Hình</th>
                        <?php } ?>
                        <th class="align-middle" style="width:30%">Tiêu đề</th>
                        <?php if (isset($config['news'][$type]['gallery_attribute']) && count($config['news'][$type]['gallery_attribute']) > 0) { ?>
                        ...
                        <?php for ($i = 0; $i < count($items); $i++) { ?>
                            <tr>
                                ...
                                <td class="align-middle">
                                    ...
                                    ?>" data-table="news_attribute">
                                </td>
                                <?php if (isset($config['news'][$type]['show_images_attribute']) &&
                                    $config['news'][$type]['show_images_attribute'] == true) { ?>
                                    <td class="align-middle">
                                        <a href="<?= $linkEdit ?>&id=<?= $items[$i]['id'] ?>" title="<?= $items[$i]['namevi'] ?>">
                                            <?= $func->getImage(['class' => 'rounded img-preview', 'sizes' =>
                                                $config['news'][$type]['thumb_attribute'], 'upload' => UPLOAD_NEWS_L,
                                                'image' => $items[$i]['photo'], 'alt' => $items[$i]['namevi']]) ?>
                                        </a>
                                    </td>
                                <?php } ?>
                                <td class="align-middle">
                                ...
                                <?php if (isset($config['news'][$type]['gallery_attribute']) && count
                                    ($config['news'][$type]['gallery_attribute']) > 0) { ?>
                                    <td class="align-middle">
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdown-gallery">
                                                <?php foreach ($config['news'][$type]['gallery_attribute'] as $key =>
                                                               $value)
                                                { ?>
                                        ...
                                        <td class="align-middle text-center">
                                            <div class="custom-control custom-checkbox my-checkbox">
                                                <input type="checkbox" class="custom-control-input show-checkbox"
                                                       ...
                                                       data-table="news_attribute" data-id="<?= $items[$i]['id'] ?>"
