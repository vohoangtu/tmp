    ...
    <form class="validation-form" novalidate method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
        ...
        <div class="row">
                                ...
                                <div class="tab-content" id="custom-tabs-three-tabContent-lang">
                                    <?php foreach ($config['website']['lang'] as $k => $v) { ?>
                                        <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php } ?>
                                            <?php if (isset($config['news'][$type]['price']) &&
                                                $config['news'][$type]['price'] == true) { ?>
                                            <div class="form-group">
                                                <label class="d-block" for="gia">Giá bán:</label>
                                                <div class="input-group">
                                                    <input type="text" class="form-control format-price gia_ban"
                                                           name="data[price]" id="price" placeholder="Giá bán" value="<?=
                                                    (!empty($flash->has('price'))) ? $flash->get('price') :
                                                        @$item['price'] ?>">
            ...
            <div class="<?= $colRight ?>">
                ...
                <?php } ?>
                <?php if (isset($config['news'][$type]['attributes']) &&
                    !empty($config['news'][$type]['attributes'])) { ?>
                <div class="card card-primary card-outline text-sm">
                    <div class="card-header">
                    ...
                    <div class="card-body">
                        <div class="form-group-category row">
                                ...
                                <?php foreach ($config['news'][$type]['attributes'] as $attribute => $title){ ?>
                                    <div class="form-group col-xl-6 col-sm-4">
                                        <label class="d-block" for="id_tags"><?=$title?>:</label>
                                        <select name="data[id_<?=$attribute?>]" class="form-control">
                                            <option value="0">Chọn <?=$title?></option>
                                        <?php
                                                $attributes = $d->rawQuery("select id, namevi from #_news_attribute WHERE type = ?", array($attribute));
                                                foreach ($attributes as $attr){
                                        ?>
                                                <option value="<?=$attr['id']?>"
                                                    <?=$attr['id'] ==
                                                    $item['id_'. $attribute] ?
                                                        'selected':''?>><?=$attr['namevi']?></option>
                                        <?php } ?>
                                        </select>
                                    </div>
                                <?php } ?>
