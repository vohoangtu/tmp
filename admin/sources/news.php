...
if (isset($config['news'])) {
    ...
    if(!in_array($_GET['act'], ['man_attribute', 'add_attribute', 'save_attribute', 'edit_attribute',
        'delete_attribute'])){
...
$strUrl = "";
$arrUrl = array('id_list', 'id_cat', 'id_item', 'id_sub', 'id_brand', 'id_attribute');
if (isset($_POST['data'])) {
...
switch ($act) {
    ...
    case "man_attribute":
        viewAttribute();
        $template = "news/attribute/attributes";
        break;
    case "add_attribute":
        $template = "news/attribute/attribute_add";
        break;
    case "edit_attribute":
        editAttribute();
        $template = "news/attribute/attribute_add";
...
function viewAttribute()
{
    ...
    $startpoint = ($curPage * $perPage) - $perPage;
    $limit = " limit " . $startpoint . "," . $perPage;
    $sql = "select * from #_news_attribute where type = ? $where order by numb,id desc $limit";
    $items = $d->rawQuery($sql, array($type));
    $sqlNum = "select count(*) as 'num' from #_news_attribute where type = ? $where order by numb,id desc";
    $count = $d->rawQueryOne($sqlNum, array($type));
    $total = (!empty($count)) ? $count['num'] : 0;
    $url = "index.php?com=news&act=man_attribute&type=" . $type;
    $paging = $func->pagination($total, $perPage, $curPage, $url);
}
...
function editAttribute()
{
    ...
    if (empty($id)) {
        $func->transfer("Không nhận được dữ liệu", "index.php?com=news&act=man_attribute&type=" . $type . "&p=" . $curPage . $strUrl, false);
    } else {
        $item = $d->rawQueryOne("select * from #_news_attribute where id = ? and type = ? limit 0,1", array($id,
            $type));

        if (empty($item)) {
            $func->transfer("Dữ liệu không có thực", "index.php?com=news&act=man_attribute&type=" . $type . "&p=" . $curPage . $strUrl, false);
        } else {
            /* Get gallery */
            $gallery = $d->rawQuery("select * from #_gallery where id_parent = ? and com = ? and type = ? and kind = ? and val = ? order by numb,id desc", array($id, $com, $type, 'man_attribute', $type));
        }
    }
}
...
function saveAttribute()
{
    ...
    if ($data) {
        ...
        if (!empty($config['news'][$type]['slug_attribute'])) {
            if (!empty($_POST['slugvi'])) $data['slugvi'] = $func->changeTitle(htmlspecialchars($_POST['slugvi']));
            else $data['slugvi'] = (!empty($data['namevi'])) ? $func->changeTitle($data['namevi']) : '';
            if (!empty($_POST['slugen'])) $data['slugen'] = $func->changeTitle(htmlspecialchars($_POST['slugen']));
            else $data['slugen'] = (!empty($data['nameen'])) ? $func->changeTitle($data['nameen']) : '';
    ...
    /* Post seo */
    if (isset($config['news'][$type]['seo_attribute']) && $config['news'][$type]['seo_attribute'] == true) {
        $dataSeo = (isset($_POST['dataSeo'])) ? $_POST['dataSeo'] : null;
        if ($dataSeo) {
            foreach ($dataSeo as $column => $value) {
                $dataSeo[$column] = htmlspecialchars($func->sanitize($value));
    ...
    if (!empty($response)) {
        ...
        if ($id) {
            $func->redirect("index.php?com=news&act=edit_attribute&type=" . $type . "&p=" . $curPage . $strUrl . "&id=" . $id);
        } else {
            $func->redirect("index.php?com=news&act=add_attribute&type=" . $type . "&p=" . $curPage . $strUrl);
        }
    }
    ...
    if ($id) {
        ...
        $d->where('id', $id);
        $d->where('type', $type);
        if ($d->update('news_attribute', $data)) {
            /* Photo */
            if ($func->hasFile("file")) {
                $photoUpdate = array();
                $file_name = $func->uploadName($_FILES["file"]["name"]);

                if ($photo = $func->uploadImage("file", $config['news'][$type]['img_type_attribute'], UPLOAD_NEWS, $file_name)) {
                    $row = $d->rawQueryOne("select id, photo from #_news_attribute where id = ? and type = ? limit 0,1", array($id, $type));
                    ...
                    $photoUpdate['photo'] = $photo;
                    $d->where('id', $id);
                    $d->update('news_attribute', $photoUpdate);
                    unset($photoUpdate);
                }
            }
            ...
            /* SEO */
            if (isset($config['news'][$type]['seo_attribute']) && $config['news'][$type]['seo_attribute'] == true) {
                $d->rawQuery("delete from #_seo where id_parent = ? and com = ? and act = ? and type = ?", array($id, $com, 'man_attribute', $type));

                $dataSeo['id_parent'] = $id;
                $dataSeo['com'] = $com;
                $dataSeo['act'] = 'man_attribute';
                $dataSeo['type'] = $type;
                $d->insert('seo', $dataSeo);
            }

            $func->transfer("Cập nhật dữ liệu thành công", "index.php?com=news&act=man_attribute&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=news&act=man_attribute&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    } else {
        $data['date_created'] = time();

        if ($d->insert('news_attribute', $data)) {
            $id_insert = $d->getLastInsertId();
            ...
            $func->transfer("Lưu dữ liệu thành công", "index.php?com=news&act=man_attribute&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=news&act=man_attribute&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    }
}
...
function deleteAttribute()
{
    ...
    if ($id) {
        /* Lấy dữ liệu */
        $row = $d->rawQueryOne("select id, photo from #_news_attribute where id = ? and type = ? limit 0,1", array
        ($id, $type));

        if (!empty($row)) {
            /* Xóa chính */
            $func->deleteFile(UPLOAD_NEWS . $row['photo']);
            $d->rawQuery("delete from #_news_attribute where id = ?", array($id));

            /* Xóa SEO */
            $d->rawQuery("delete from #_seo where id_parent = ? and com = ? and act = ? and type = ?", array($id, $com, 'man_attribute', $type));

            /* Xóa gallery */
            $row = $d->rawQuery("select id, photo, file_attach from #_gallery where id_parent = ? and kind = ? and com = ?", array($id, 'man_attribute', $com));
            ...
            if (count($row)) {
                ...
                $d->rawQuery("delete from #_gallery where id_parent = ? and kind = ? and com = ?", array($id, 'man_attribute', $com));
            }

            $func->transfer("Xóa dữ liệu thành công", "index.php?com=news&act=man_attribute&type=" . $type . "&p=" . $curPage . $strUrl);
        } else {
            $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=news&act=man_attribute&type=" . $type . "&p=" . $curPage . $strUrl, false);
        }
    } elseif (isset($_GET['listid'])) {
        $listid = explode(",", $_GET['listid']);
        ...
        for ($i = 0; $i < count($listid); $i++) {
            ...
            /* Lấy dữ liệu */
            $row = $d->rawQueryOne("select id, photo from #_news_attribute where id = ? and type = ? limit 0,1",
                array($id, $type));
